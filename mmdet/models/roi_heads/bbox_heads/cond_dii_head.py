...
class CondDIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        super(CondDIIHead, self).__init__(
        ...
        self.embeds = nn.Embedding(self.dim_words, self.dim_embedding)
        for _ in range(num_cls_fcs):
            if _ == 0:
                # add anatomy
                self.cls_fcs.append(
                    nn.Linear(in_channels + self.dim_embedding, in_channels, bias=False))
            else:
                self.cls_fcs.append(
    ...
    def init_weights(self):
        ...
        super(CondDIIHead, self).init_weights()
    ...
    @auto_fp16()
    def forward(self, roi_feat, proposal_feat, anatomy=None):
        ...
        # print(anatomy)
        # print(len(anatomy))
        # print(anatomy[0].shape)
        anatomy = torch.cat(anatomy, 0).reshape(-1, 1)# (bs, 1)
        # print("anatomy.shape", anatomy.shape)
        word_feat = self.embeds(anatomy) # (bs, 1, dim_embed)
        # print("word_feat.shape embeds", word_feat.shape)
        word_feat = word_feat.repeat(1, num_proposals, 1) # (bs, num_proposals, dim_embed)
        # print("word_feat.shape repeat", word_feat.shape)
        word_feat = word_feat.reshape(-1, self.dim_embedding) # (bs * num_proposals, dim_embed)
        # print("word_feat.shape reshape", word_feat.shape)
        # anatomy = F.repeat(anatomy, )
        # print(cls_feat.shape, word_feat.shape)
        cls_feat = torch.cat([cls_feat, word_feat], 1) # (bs * num_proposals, in_channels + dim_embed)
