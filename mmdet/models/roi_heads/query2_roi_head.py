...
class Query2RoIHead(QueryRoIHead):
    r"""
...
#         assert mask_head is not None
        ...
        super(Query2RoIHead, self).__init__(
            num_stages,
            stage_loss_weights,
        ...
        if train_cfg is not None:
            for stage in range(num_stages):
                assert isinstance(self.bbox_sampler[stage], PseudoSampler), \
                    'Query2RoIHead only support `PseudoSampler`'

    def _bbox_forward(self, stage, x, rois, global_rois, object_feats, global_feats, img_metas):
        ...
        bbox_feats = bbox_roi_extractor(x[:bbox_roi_extractor.num_inputs],
                                        rois)
        cls_score, bbox_pred, global_score, global_bbox_pred, object_feats, global_feats, attn_feats = bbox_head(
        ...
        proposal_list = self.bbox_head[stage].refine_bboxes(
            ...
            img_metas)
        proposal_global_list = self.bbox_head[stage].refine_bboxes(
            global_rois,
            global_rois.new_zeros(len(global_rois)),  # dummy arg
            global_bbox_pred.view(-1, global_bbox_pred.size(-1)),
            [global_rois.new_zeros(global_feats.size(1)) for _ in range(num_imgs)],
            img_metas)
        bbox_results = dict(
            cls_score=cls_score,
            decode_bbox_pred=torch.cat(proposal_list),
            global_score=global_score,
            decode_global_bbox_pred=torch.cat(proposal_global_list),
            ...
            detach_proposal_global_list=[item.detach() for item in proposal_global_list])
    ...
    def forward_train(self,
        ...
        imgs_whwh_global = imgs_whwh.repeat(1, 1, 1)
        imgs_whwh = imgs_whwh.repeat(1, num_proposals, 1)
        all_stage_bbox_results = []
        proposal_list = [proposal_boxes[i] for i in range(len(proposal_boxes))]
        proposal_global_list = [proposal_global_box[i] for i in range(len(proposal_global_box))]
        ...
        for stage in range(self.num_stages):
            ...
            global_rois = bbox2roi(proposal_global_list)
            bbox_results = self._bbox_forward(
                stage, x, rois, global_rois, object_feats, global_feats, img_metas)
            ...
            proposal_global_list = bbox_results['detach_proposal_global_list']
            ...
            decode_global_bbox_pred = bbox_results['decode_global_bbox_pred']
            ...
            single_stage_loss = self.bbox_head[stage].loss(
                ...
                *bbox_targets,
                global_labels, 
                decode_global_bbox_pred, # query2
                gt_bboxes, # query2
                gt_labels, # query2
                imgs_whwh=imgs_whwh,
                imgs_whwh_global=imgs_whwh_global)
    ...
    def simple_test(self,
        ...
        for stage in range(self.num_stages):
            rois = bbox2roi(proposal_list)
            global_rois = bbox2roi(proposal_global_list)
            bbox_results = self._bbox_forward(stage, x, rois, global_rois, object_feats, global_feats,
                                              img_metas)
            object_feats = bbox_results['object_feats']
            cls_score = bbox_results['cls_score']
            global_score = bbox_results['global_score']
        ...
        for img_id in range(num_imgs):
            ...
            bbox_pred_per_img = proposal_list[img_id][topk_indices //
                                                      num_classes]
            global_bbox_pred_per_img = proposal_global_list[img_id]
            if rescale:
                scale_factor = img_metas[img_id]['scale_factor']
                bbox_pred_per_img /= bbox_pred_per_img.new_tensor(scale_factor)
                global_bbox_pred_per_img /= global_bbox_pred_per_img.new_tensor(scale_factor)
            det_bboxes.append(
                torch.cat([bbox_pred_per_img, scores_per_img[:, None]], dim=1))
            det_labels.append(labels_per_img)
            global_det_bboxes.append(global_bbox_pred_per_img)
        ...
        if self.with_mask:
            results = list(
                zip(ms_bbox_result['ensemble'], ms_segm_result['ensemble'], global_results, global_det_bboxes))
        else:
            results = list(
                zip(ms_bbox_result['ensemble'], global_results, global_det_bboxes))
