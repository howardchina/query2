...
class VladGlobalEmbeddingRPNHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(VladGlobalEmbeddingRPNHead, self).__init__(init_cfg)
    ...
    def _init_layers(self):
        ...
        self.init_global_features = nn.Embedding(
            self.dim_global, self.num_clusters * self.proposal_feature_channel)
    ...
    def init_weights(self):
        ...
        image.
        """
        super(VladGlobalEmbeddingRPNHead, self).init_weights()
    ...
    def forward_dummy(self, img, anatomy, img_metas):
        ...
        if self.use_global_forward:
            ...
            init_global_features = self.init_global_features(anatomy)
        else:
            num_imgs = len(img[0])
            init_global_features = self.init_global_features.weight.clone()
            init_global_features = init_global_features[None].expand(
                num_imgs, *init_global_features.size())
        init_global_features = init_global_features.reshape(-1, self.num_clusters, self.proposal_feature_channel)
        
        proposals, init_proposal_features, imgs_whwh = self._decode_init_proposals(img, img_metas)
        return proposals, init_proposal_features, init_global_features, imgs_whwh
    ...
    def aug_test_rpn(self, imgs, anatomies, img_metas):
        ...
        for img, anatony, img_meta in zip(imgs, anatomies, img_metas):
            proposal_boxes, proposal_features, global_features, imgs_whwh = self.simple_test_rpn(img, anatony, img_meta)
        ...
        return aug_proposal_boxes, aug_proposal_features, aug_global_features, aug_imgs_whwh
